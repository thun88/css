...
module.exports = (gulp, gconfig, postCssPlugins, svgHtml) => {
  ...
  gulp.task('src:md:compile', ['svg:store'], () => {
    ...
    const inlineIcons = fs.readFileSync(`${gconfig.paths.packages}/${gconfig.project.prefix}-icon/dist/inline-icons.svg`, 'utf-8');
    ...
    gulp.src([
      `${gconfig.paths.src.packages}/${gconfig.project.prefix}-components/dist/*.min.css`,
      `${gconfig.paths.src.packages}/${gconfig.project.prefix}-components/dist/*.min.css.map`
